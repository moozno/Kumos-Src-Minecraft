...
const { clientID, webhookURL, port1, port2 } = JSON.parse(await readFile(new URL("./config.json", import.meta.url)));
...
async function init() {
  ...
  const privateKey = fs.readFileSync('/etc/letsencrypt/live/domain-here.com/privkey.pem', 'utf8');
  const certificate = fs.readFileSync('/etc/letsencrypt/live/domain-here.com/fullchain.pem', 'utf8');
  ...
  httpServer.listen(port1, () => {
    console.log(`--------------------------------------------------`);
    console.log(`Started the server on ${port1}`);
    console.log(`--------------------------------------------------\n`);
  });

  httpsServer.listen(port2, () => {
    console.log(`--------------------------------------------------`);
    console.log(`Started the server on ${port2}`);
    console.log(`--------------------------------------------------\n`);
  });
}
